...
j1=$(sbatch --job-name bwa.$1 --mem=30g --cpus-per-task 10 ~/git/NGS_genotype_calling/exome_workflow_v02/realign_NISC_bam_with_bwa.py $1)
############

############
# BAM processing
# sort, mark duplicates, and index bam
j2=$(sbatch --job-name sortMarkIndex.$1 --dependency=afterok:$j1 --mem=20G ~/git/NGS_genotype_calling/exome_workflow_v02/run_Picard_SortMarkIndexBam.sh ${1%.bam}.bwa-mem.b37.bam)
############
...
j3=$(sbatch --job-name callGVCF.$1 --dependency=afterok:$j2 --mem=16g --time=24:00:00 ~/git/NGS_genotype_calling/exome_workflow_v02/run_GATK_callGVCFs.sh ${1%.bam}.bwa-mem.b37.sorted.markDup.bam $2)
############
